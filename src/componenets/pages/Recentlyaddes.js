...
const RecentlyAdded = () => {
  ...
  return (
    <section className='home'>
        <div className='banner' style={{backgroundImage:upcomingMovies[0] ? `url(${`${imgUrl}/${upcomingMovies[0].poster_path}`})`
        : 'black'}} >
        
        {upcomingMovies[0] && (
            <span className='title'>{upcomingMovies[0].original_title }</span>     )
        }
        {upcomingMovies[0] && (
            <p className='para'>{upcomingMovies[0].overview }</p>     )
        }
